import { mail_data, me } from './mail_data.js';
import { month_31, getToday } from './date.js';
...
$(function () {
	...
	$('.button.send').click(() => {
		...
		let receiver = [];
		receiver_email.forEach((element) =>
			receiver.push(element.split('@')[0][0].toUpperCase() + element.split('@')[0].slice(1))
		);
...
function viewMail(mail) {
	...
	let createReceiverSpan = (display_name, email) => {
		let receiver_dom = `<span class="display-name">${
			display_name == me.name ? display_name + ' (Me)' : display_name
		}</span>
